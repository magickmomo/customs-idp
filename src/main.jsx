...
function PackTable({packs,onOpen,onAssign}){return <div className="table-wrap"><table><thead><tr><th>PACK</th><th>CUSTOMER</th><th>OWNER</th><th>TICKET</th><th>DOCUMENTS</th><th>EXPORT</th><th>DESTINATION</th><th>INVOICE TOTAL</th><th>GROSS KG</th><th>STATUS</th><th>CONFIDENCE</th><th>RECEIVED</th><th></th></tr></thead><tbody>{packs.map(p=>{const d=p.extractedData||{};return <tr key={p.id} onClick={()=>onOpen(p)}><td><b>{p.id}</b></td><td>{p.customer}</td><td><select className="owner-select" value={p.assignedTo||"Unassigned"} onClick={e=>e.stopPropagation()} onChange={e=>onAssign?.(p.id,e.target.value)}><option>Unassigned</option><option>Liam Wingrove</option><option>Data Processor 1</option><option>Data Processor 2</option><option>Muhammad Amer</option></select></td><td>{p.ticket||"—"}</td><td>{p.docs} documents</td><td>{d.countryOfExport||"—"}</td><td>{d.sourceCountryOfDestination||d.countryOfDestination||"—"}</td><td>{d.currency&&d.totalInvoiceValue!=null?`${d.currency} ${Number(d.totalInvoiceValue).toLocaleString(undefined,{minimumFractionDigits:2})}`:"—"}</td><td>{d.totalGrossWeight!=null?d.totalGrossWeight:"—"}</td><td><Status status={p.status}/></td><td><div className="confidence"><span>{p.confidence}%</span><div><i style={{width:p.confidence+"%"}}></i></div></div></td><td>{p.received}</td><td><button className="row-btn" onClick={e=>e.stopPropagation()}><MoreHorizontal size={17}/></button></td></tr>})}</tbody></table></div>}
